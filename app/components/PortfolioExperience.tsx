...
export default function PortfolioExperience({ projectItems }: { projectItems: Project[] }) {
  ...
  async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();const form=e.currentTarget;setFormState("sending");const body=Object.fromEntries(new FormData(form));try{const r=await fetch("/api/contact",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});if(!r.ok)throw new Error();form.reset();setFormState("sent")}catch{setFormState("error")}}
